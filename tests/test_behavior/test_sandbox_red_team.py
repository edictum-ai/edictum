"""Red team regression tests for sandbox contracts.

Every test case comes from a real red team session against a Nanobot agent.
These are the actual bypasses that L1 regex contracts couldn't close.
"""
...
class TestRedTeamComposition:
    """Deny contracts fire before sandbox -- belt and suspenders."""

    def test_rm_rf_caught_by_deny(self, guard):
        """rm -rf / is caught by deny-destructive, not sandbox."""
    ...
    def test_reverse_shell_caught_by_deny(self, guard):
        """bash -i caught by deny-shells, not sandbox."""
