...
class TestSymlinkEscape:
    def test_symlink_inside_allowed_pointing_outside(self, sandbox_tmpdir):
        tool_call = create_envelope("Read", {"path": sandbox_tmpdir["escape_path"]})
    ...
    def test_symlink_chain_resolved(self, sandbox_tmpdir):
        tool_call = create_envelope("Read", {"path": sandbox_tmpdir["chain_path"]})
        paths = _extract_paths(tool_call)
        for p in paths:
            assert not p.startswith(sandbox_tmpdir["allowed"])

    def test_normal_file_still_allowed(self, sandbox_tmpdir):
        tool_call = create_envelope("Read", {"path": sandbox_tmpdir["safe_file"]})
        paths = _extract_paths(tool_call)
        for p in paths:
    ...
    def test_dotdot_still_denied(self, sandbox_tmpdir):
        ...
        tool_call = create_envelope("Read", {"path": escape})
        paths = _extract_paths(tool_call)
        for p in paths:
            assert not p.startswith(sandbox_tmpdir["allowed"])

    def test_nonexistent_path_no_crash(self):
        tool_call = create_envelope("Read", {"path": "/tmp/does/not/exist/file.txt"})
        paths = _extract_paths(tool_call)
        assert len(paths) >= 1
...
class TestSandboxContractWithSymlinks:
    def test_sandbox_denies_symlink_escape(self, sandbox_tmpdir):
        ...
        sandbox_fn = _compile_sandbox(rule, "enforce")
        tool_call = create_envelope("Read", {"path": sandbox_tmpdir["escape_path"]})
        decision = sandbox_fn(tool_call)
