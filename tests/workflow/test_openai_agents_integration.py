...
@pytest.mark.asyncio
async def test_openai_agents_adapter_records_read_evidence_only_after_success():
    guard = Edictum.from_yaml_string(_BUNDLE, backend=MemoryBackend(), workflow_content=_WORKFLOW)
    adapter = OpenAIAgentsAdapter(guard, session_id="openai-workflow-failure")

    assert await adapter._pre("Read", {"path": "spec.md"}, "read-1") is None
    await adapter._post("read-1", "Error: read failed")

    state = await _workflow_state(guard, "openai-workflow-failure")
