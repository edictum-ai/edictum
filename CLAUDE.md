...
- v0.15.0 (unreleased): Ed25519 bundle signature verification — `verify_signatures` + `signing_public_key` params on `from_server()`, `verify_bundle_signature()` and `BundleVerificationError` in `edictum.server`, `edictum[verified]` extra (PyNaCl)
